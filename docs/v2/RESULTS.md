...
- **Accessibility**: an automated scan (axe-core) on Jarvis's home, Memory, Weekly and Settings screens found one real class of problem, low-contrast form fields, which turned out to be the page's gradient background confusing the checker (the text really is light on dark); the body now has a solid fallback colour and all four screens scan clean (0 violations). By hand: repeated controls now say which note they act on ("Forget: <note>", "Helpful: <note>"), Jarvis's notes are a polite live region so they are announced when they appear, the budget bar is a labelled progress bar, headings run h1/h2 on every screen, every control is a native button, link or field (so it is reachable by keyboard), focus rings and reduced-motion handling were already global. Not done: a pass with a real screen reader, and a keyboard-only walk on a real phone.
- **Security review of the one-address design**: nothing in the source turns text into code or markup (a test scans everything, and a check proves it would catch one); the content policy is strict and held by a test (scripts, connections, framing, forms and base only from this address; the only outside hosts are the two font hosts); every server route checks the access code (or the cron secret) before doing anything, and a test lists the routes so a new one cannot slip in; the service worker never caches an API answer. The review found one gap and fixed it: a link target that was not a path on this address (a script address, another site) used to pass through the link helper unchanged, and now becomes the home page (tested). Recorded in `docs/SECURITY.md`.
- **Routing cases for version 2**: six more (a request to remember a preference and to forget a note must call the memory tools; asking for an opinion, an ordinary question about running, feedback about a note, and "be quiet for a few days" must call no action), on top of the five memory cases and the "lasting fact is offered, a one-off is not" cases added in Phase 6: 146 phrases in all. The six new ones have **not** been run against the real model yet.
- **Documents**: README (layout, what Jarvis is now, the commands), SPEC (section 9.1: new events, memory selection, observations, the AI purposes, routes), SECURITY (a section on the one-address design and the updated checklist).
- **One-command health check** (`npm run health`) was built in Phase 9; it is the "written checklist plus scripts" of P10.5.
- **Left for the final run (P10.6), because it needs your yes**: the routing eval (about half a dollar), the preview deployment and smoke pass, deployment and live verification.

## Routing eval on the final build (run on request, 2026-09-21)

- **First full run: 140 of 150 (93.3%), under the 95% bar.** The caching restructure was ruled out as the cause: the same phrases failed nearly the same way on the handler from before it. The cause was earlier prompt changes (Phase 5 to 7) that made him chatty, asking "how long did it take?" or "shall I log it?" instead of logging, plus three of my own new cases that expected the wrong thing (nothing to forget in an account with no notes, a second dentist visit).
- **Fixes**: instructions now say to log what the user says they did without asking, that "did/finished/made" mean it happened, that a second appointment is fine, that money coming in is never a purchase (with the refund example), that a decision journal entry goes to the Compass tracker, that "be quiet" points to the Jarvis speaks-up setting instead of pausing areas, and that he offers to remember by calling the memory tool, not by asking in words. One test case was corrected.
- **Second full run: 148 of 150 (98.7%)**: single 70/70, multi 9/9, typo 10/10, ambiguous 15/15, adversarial 14/14, new 30/32. About 1.0 s median, 1.9 s at the 95th percentile, $0.0018 a request. The two misses (a refund logged as a negative purchase; a vegetarian offer asked in words) were then fixed with two more sentences, and those four phrases pass (4/4).
- **Not verified on the final wording**: a third full run failed partway because **the Anthropic account ran out of credit** (every call answered "credit balance too low"). Its valid part (single, multi, typo 9/10 with the dentist typo, new 32/32) passed; the 7 last ambiguous and all 14 adversarial phrases were not actually tested on the final wording.
- **After credit was added and the monthly cap raised to $5 (user's decision), a fourth full run: 149 of 150 (99.3%)**: single 70/70, multi 9/9, typo 9/10, ambiguous 15/15, adversarial 14/14, new 32/32. Median 1.0 s, 1.9 s at the 95th percentile, $0.0018 a request, $0.28 for the run. The one miss is "add dentist tomorow at 3pm": he asked whether it was a new visit or a reschedule of the existing one instead of adding it, a reasonable clarifying question rather than a wrong action, left as is. **TP4 and P9.4 are now satisfied.**

## Going live (2026-09-21)

- User confirmed: today's backup export is sufficient, and to merge `v2` into `master` and deploy to production tonight rather than stopping at a preview.
- AI monthly cap raised from $2 to $5 by the user's own decision (Vercel env `AI_MONTHLY_CAP_USD`, set in production, preview and development), effective from this deployment. The user asked to be told the cost and asked first for anything over 10 cents; this is now standing guidance, not just for this deploy.
- Preview address `https://evergrove-omgthopyq-focus23.vercel.app` was health-checked earlier (58/58) and walked through locally (16 of 17 acceptance steps directly verified; step 3 install and step 17 the 9pm notification need the live site or a real device).

- **Deployed.** `master` merged from `v2` (clean, no conflicts), pushed to `galifra/evergrove`, auto-deployed by Vercel to production: `https://evergrove-howidnh49-focus23.vercel.app`, live at `https://evergrove-neon.vercel.app`. `npm run health -- --url https://evergrove-neon.vercel.app`: **58 of 58 checks pass** (every app page and manifest, Jarvis's four screens, `/sw.js` served fresh, all four APIs correctly refuse a request with no code).
- **Rollback stays ready**: the previous production deployment `evergrove-3pifn1hdv-focus23.vercel.app` (tag `v1-final`, commit `f679c7e`) is still listed Ready. Keep it, and don't delete the tag, for 7 days per `docs/v2/CUTOVER.md`.
- **Still needed from the user**: open the live site once (P9.8), run the acceptance walkthrough on it (steps 3 install and 17 the 9pm notification need the real device/site), and confirm tonight's briefing arrives (P9.9).
