...
export function appSpecMarkdown(req) {
  const lines = [
    ...
    '- A module with its own address (`/<id>`) and its own installable page, usable without MOXIE.',
    ...
    '- A manifest listing its actions (with a permission tier each: auto, ask first, or suggest only) so MOXIE can use it.',
    ...
    '- Tests, including routing phrases for MOXIE, and it must work offline.',
    '',
