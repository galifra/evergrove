...
import { deriveToday, snoozeUntil, visibleToday } from '@evergrove/rules/today.js'
...
export default function TodayCard() {
  ...
  return (
    <section className="mt-5 w-full max-w-xl rounded-2xl bg-white/[0.04] border border-white/10 p-4" aria-label="Today">
      ...
      {shown.length === 0 ? (
        <p className="text-sm text-white/55 italic">Nothing pressing. Enjoy the day.</p>
      ) : (
          ...
          {shown.map((i) => (
            <li key={i.id} className="flex items-center gap-2 group">
              ...
              <button onClick={() => go(i.route)} className="flex-1 min-w-0 text-left text-sm text-white/80 hover:text-white py-1 flex items-center gap-1">
                ...
                <ChevronRight size={13} className="shrink-0 text-white/55" />
              </button>
              <button onClick={() => dismiss(i)} className="p-1 rounded text-white/55 hover:text-white/70" aria-label="Dismiss for now">
                <X size={13} />
              </button>
            </li>
          ))}
